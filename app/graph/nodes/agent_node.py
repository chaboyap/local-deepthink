...
async def _agent_node_logic(state: GraphState, services: ServiceContext, node_id: str) -> dict:
    ...
    if len(json.dumps(agent_memory)) > settings.hyperparameters.agent_memory_limit_bytes:
         ...
         summarizer_chain = prompt_service.create_chain(services.summarizer_llm, "memory_summarizer")
...
"""
    ...
    for attempt in range(max_attempts):
        current_llm, current_config, model_name = (services.llm, state['llm_config'], settings.llm_providers.main_llm.model_name) \
            if attempt == 0 else \
            (services.synthesizer_llm, state['synthesizer_llm_config'], settings.llm_providers.synthesizer_llm.model_name)
